...
@router.get("/overview")
def waybill_overview(
    waybill: str | None = Query(default=None, description="按运单号汇总测点温度概览"),
) -> dict[str, Any]:
    """测点温度概览：按运单汇总各测点的实时温度、上下限与状态，偏高幅度大的排最前。"""
    return service.summarize_waybill(waybill)
